...
static void example_lvgl_touch_cb(lv_indev_t *indev, lv_indev_data_t *data)
{
    ...
    if (touchpad_pressed && touchpad_cnt > 0) {
        int x = (touchpad_x[0] - 18) * (239 - 0) / (299-18)+0;
        int y = (touchpad_y[0] - 19) * (319 - 0) / (222-19)+0;
        data->point.x = x;
        data->point.y = y;
...
void init_mylvgl(void)
{
    ...
    xTaskCreate(example_lvgl_port_task, "LVGL", 8*1024, NULL, EXAMPLE_LVGL_TASK_PRIORITY, NULL);
